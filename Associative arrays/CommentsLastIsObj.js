function solve(input){
    ...
    */ 
    ...
    for (const line of input) {
        ...
        if((tokens[0]) == 'user'){
            ...
            if(articlesList.includes(articleName) && usersList.includes(userPosting)){
                ...
                articlesObj[articleName][userPosting][commentTitle] = commentContent;
    ...
    for (const [article, userObj] of articleCountEntries) {
        let usersEntries = Object.entries(articlesObj[article]); //[[user, commentObj ]... ]
        usersEntries.sort((a,b) => a[0].localeCompare(b[0]));
        console.log(`Comments on ${article}`);

        for (const [user, commentObj] of usersEntries) {
            
            let keysAndValues = Object.entries(articlesObj[article][user]);
